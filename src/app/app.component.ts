...
export class AppComponent implements OnInit {
  ...
  // 1 base line
  Base_line = Highcharts;
  Base_line_option: any

  // 2 base line
  Base_line2 = Highcharts;
  Base_line_option2: any

  // With annotation
  withannotation = Highcharts;
  withannotation_option: any;
  ...
  stockchart() {
    this.Stock1_option = {
      ...
      {
        type: 'flags', showInLegend: false,
        shape: 'circlepin',
        data: [{ x: 100, text: '3ssssss', title: '3sssssss' },
        ]
        // shape: 'circlepin',
      },
      {
  ...
  addHighlight(e: any) {
    ...
    charts.forEach((chart, index) => {
      if (chart) {
        ...
        point = (chart?.series[0] as any).searchPoint(e, true);
        if (point) {
          point.highlight(e);
        }
      }
   ...
   // mouse Moment stock
   addHighlightStock(e: any) {
    // console.log('e: ', e);
    let chart, point, i, event;
    const charts = Highcharts1.charts;
    charts.forEach((chart, index) => {
      if (chart) {
        chart = charts[index];
        // Find coordinates within the chart
  ...
  // Zoom work stock 
  syncExtremesstock(e: any) {
    // this.syncExtremes(e)
    var thisChart = Highcharts1.charts
    
    const shouldShowResetZoom = !(e.min === undefined && e.max === undefined);

    if (e.trigger !== 'syncExtremes') { // Prevent feedback loop
      Highcharts1.each(Highcharts1.charts, function (chart: any) {
        if (chart !== thisChart) {
          if (chart.xAxis[0].setExtremes) { // It is null while updating
            chart.xAxis[0].setExtremes(
              e.min,
      ...
      // Highcharts1.charts.forEach((chart:any) => {
      //   if (chart !== thisChart) {
      //     if (chart.xAxis[0].setExtremes) {
      //       // It is null while updating
      //       chart.xAxis[0].setExtremes(
